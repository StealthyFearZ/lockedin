...
def export_as_csv(modeladmin, request, queryset): # admin selection objects in as queryset
    fields = queryset.model._meta.fields # get row fields
    ...
    response['Content-Disposition'] = f'attachment; filename="{queryset.model._meta.model_name}.csv"' # set to attachment (download, not view)
...
@admin.register(Application) # alternative way of registering with decorator
class ApplicationAdmin(admin.ModelAdmin):
    actions = [export_as_csv] # add to action
